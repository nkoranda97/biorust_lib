...
pub fn dna_distance_matrix(
    ...
    for i in 0..n {
        for j in (i + 1)..n {
            let (ts, tv, valid) = count_dna_differences(seqs[i], seqs[j]);
            if valid == 0 {
                return Err(BioError::NoValidSites { i, j });
            }

            let d = match model {
                DnaDistanceModel::PDistance => (ts + tv) as f64 / valid as f64,
...
pub fn protein_distance_matrix(
    ...
    for i in 0..n {
        for j in (i + 1)..n {
            let (mismatches, valid) = count_protein_differences(seqs[i], seqs[j]);
            if valid == 0 {
                return Err(BioError::NoValidSites { i, j });
            }
            ...
            let d = match model {
                ProteinDistanceModel::PDistance => p,
